...
class GenMotifGraph(object):
    ...
    def gen_Motif(self,dir):
            ...
            for ii in range(len(all_motifs)):
                for jj in range(ii + 1, len(all_motifs)):
                    ...
                    for motif in [motif_a,motif_b]:
                        if len(motif) > 2: 
                            weight = tuple(self.find_ring_weights(motif, g))##motif的属性
                            ring = []
                            for i in range(len(motif)):
                                ring.append(node_labels[motif[i]])
                            cycle = (tuple(ring), weight)
        ...
        ####==============================================================####
        mm_graph = Data(x = mm_g_x,edge_index=mm_edge_index,edge_attr=mm_edge_attr)
        

        self.save_data(dir,'MM_graph_large.pkl',mm_graph)
        self.save_data(dir,'motif_graph_data_large.pkl',Motif_datas)
        self.save_data(dir,'node_motif_graph_large.pkl',node_Motif_datas)
        self.save_data(dir,'motif_vocab_large.pkl',self.vocab)
        self.save_data(dir,'motif_vocab_reverse_large.pkl',self.vocab_reverse)
        self.save_data(dir,'motif_graph_large.pkl',g_lists)
        self.save_data(dir,'motifs_large.pkl',self.motifs)
