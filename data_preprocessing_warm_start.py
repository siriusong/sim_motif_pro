...
def generate_drug_data(mol_graph, atom_symbols):
    ...
    data = CustomData(x=features, edge_index=new_edge_index, edge_attr=edge_feats,mol_graph = mol_graph )
...
def split_data(args):
    ...
    cv_split = StratifiedShuffleSplit(n_splits=args.n_folds, test_size=args.test_ratio)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='twosides', choices=['drugbank', 'twosides'],
                            help='Dataset to preprocess.')
    parser.add_argument('-n', '--neg_ent', type=int, default=1, help='Number of negative samples')
    parser.add_argument('-o', '--operation', type=str, default='split', choices=['all', 'generate_triplets', 'drug_data', 'split'], help='Operation to perform')
    parser.add_argument('-t_r', '--test_ratio', type=float, default=0.2)
    parser.add_argument('-n_f', '--n_folds', type=int, default=2)
