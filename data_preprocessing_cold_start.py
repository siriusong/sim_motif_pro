...
class CustomData(Data):
    ...
    # Replace with "def __inc__(self, key, value, *args, **kwargs)"
        ...
        # In case of "TypeError: __inc__() takes 3 positional arguments but 4 were giv
#TODO: Put a dictionary containing dataset_names with their corresponding column names.
...
def generate_drug_data(mol_graph, atom_symbols):
    ...
    data = CustomData(x=features, edge_index=new_edge_index, line_graph_edge_index=line_graph_edge_index,
                      edge_attr=edge_feats, mol_graph=mol_graph)

    return data

def load_drug_mol_dat(args):

    # TODO: Places that need to be modified
...
def generate_pair_triples(args):
    ...
    for id1, id2, relation in zip(data[args.c_id1], data[args.c_id2],  data[args.c_y]):
        ...
        # Drugbank dataset 1-based indexed here to substract by one
    ...
    # save_data(pair_triples, 'pair_pos_neg_triples.pkl', args)
...
def _normal_batch( h, t, r, neg_size, data_statistics, drug_ids, desc):
    ...
    #TODO Should the remaining drug/ent, the one not replaced, be also excluded from generated corrupted ones
    #TODO I guess it can be included because there is no interaction between an entity and itself, an entity and itself 
    # might be a good pair forming a negative sample 
...
parser.add_argument('-d', '--dataset', type=str,default='drugbank', choices=['deepddi', 'drugbank', 'twosides'],
...
parser.add_argument('-o', '--operation', type=str, default='all', choices=['all', 'generate_triples', 'drug_data'], help='Operation to perform')
...
args.dirname = 'data/preprocessed/cold_start'
